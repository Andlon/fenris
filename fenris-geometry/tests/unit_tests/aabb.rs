...
proptest! {
    ...
    fn aabb_furthest_point_2d(p in point2(), aabb in aabb2()) {
        ...
        // Need this check to make sure we haven't just found a point way outside the
        // bounding box
        let attained_by_atleast_one_corner = aabb.corners_iter()
            .any(|corner| distance_squared(&p, &q) == distance_squared(&p, &corner));
        prop_assert!(attained_by_atleast_one_corner);
    }

...
/// The result of a call to [`AxisAlignedBoundingBox::furthest_point_to`].
pub struct FurthestPoint<T, D>
where
    T: Scalar,
    D: DimName,
    DefaultAllocator: Allocator<T, D>
{
    /// The point in the bounding box furthest away from the query point.
    pub point: OPoint<T, D>,
    /// The squared distance to the point.
    pub dist2: T
}

impl<T, D> FurthestPoint<T, D>
where
    T: Real,
    D: DimName,
    DefaultAllocator: Allocator<T, D>

...
pub(crate) fn index_set_nth_power_iter<const N: usize>(n: usize) -> impl Iterator<Item = [usize; N]> {
    let mut multi_idx = [0usize; N];
    let mut i = 0;
    std::iter::from_fn(move || {
        if i < n.pow(N as u32) {
            let item = Some(multi_idx);
            for j in 0..N {
                if multi_idx[j] + 1 < n {
                    multi_idx[j] += 1;
                    break;
...
/// Returns an iterator that iterates over the elements of $S^N$, where $S$ is the input set.
pub(crate) fn set_nth_power_iter<'a, T: Default + Clone, const N: usize>(
    set: &'a [T],
) -> impl 'a + Iterator<Item = [T; N]> {
    index_set_nth_power_iter(set.len()).map(move |multi_idx| multi_idx.map(|idx| set[idx].clone()))
}

#[cfg(test)]
mod tests {
    use crate::util::set_nth_power_iter;

    fn collect_set_nth_power<T: Default + Clone, const N: usize>(set: &[T]) -> Vec<[T; N]> {
        set_nth_power_iter(set).collect()
    }

    #[test]
    fn test_set_nth_power_iter() {
        {
            // Check empty set for different powers
            // TODO: I'm not sure if this is correct or not: ∅^0 = { ∅ }, but ∅^1 = ∅ ...
            assert_eq!(collect_set_nth_power::<i32, 0>(&[]), vec![[]]);
            assert_eq!(collect_set_nth_power::<i32, 1>(&[]), Vec::<[i32; 1]>::new());
            assert_eq!(collect_set_nth_power::<i32, 2>(&[]), Vec::<[i32; 2]>::new());
            assert_eq!(collect_set_nth_power::<i32, 3>(&[]), Vec::<[i32; 3]>::new());
        }

        {
            // 1-element set
            assert_eq!(collect_set_nth_power::<i32, 0>(&[1]), vec![[]]);
            assert_eq!(collect_set_nth_power::<i32, 1>(&[1]), vec![[1]]);
            assert_eq!(collect_set_nth_power::<i32, 2>(&[1]), vec![[1, 1]]);
            assert_eq!(collect_set_nth_power::<i32, 3>(&[1]), vec![[1, 1, 1]]);
        }

        {
            // 2-element set
            assert_eq!(collect_set_nth_power::<i32, 0>(&[5, 8]), vec![[]]);
            assert_eq!(collect_set_nth_power::<i32, 1>(&[5, 8]), vec![[5], [8]]);
            assert_eq!(
                collect_set_nth_power::<i32, 2>(&[5, 8]),
                vec![[5, 5], [8, 5], [5, 8], [8, 8]]
            );
            assert_eq!(
                collect_set_nth_power::<i32, 3>(&[5, 8]),
                vec![
                    [5, 5, 5],
                    [8, 5, 5],
                    [5, 8, 5],
        ...
        {
            // 3-element set
            assert_eq!(collect_set_nth_power::<i32, 0>(&[2, 3, 4]), vec![[]]);
            assert_eq!(collect_set_nth_power::<i32, 1>(&[2, 3, 4]), vec![[2], [3], [4]]);
            assert_eq!(
                collect_set_nth_power::<i32, 2>(&[2, 3, 4]),
                vec![[2, 2], [3, 2], [4, 2], [2, 3], [3, 3], [4, 3], [2, 4], [3, 4], [4, 4]]
            );
        }

        {
            // Check that it works as expected with bools
            assert_eq!(collect_set_nth_power(&[true, false]), vec![[true], [false]]);
            assert_eq!(
                collect_set_nth_power(&[true, false]),
